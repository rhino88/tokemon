...
impl DroidProvider {
    pub fn new() -> Self {
        Self {
            base_dir: paths::home_dir().join(".droid/sessions"),
        }
    }
}
...
impl super::Provider for DroidProvider {
    fn name(&self) -> &str {
        "droid"
    }
    ...
    fn parse_file(&self, path: &Path) -> Result<Vec<UsageEntry>> {
        ...
        for line in reader.lines() {
            ...
            let parsed: DroidLine = match serde_json::from_str(&line) {
                Ok(p) => p,
                Err(_) => continue,
            };
